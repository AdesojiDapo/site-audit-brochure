...
2. **A site-wide review.** Extends the audit beyond the homepage to your real URLs (login page, REST API, sitemap, feed, common attack-surface paths).
...
- Not a penetration test. No exploits, no credentials guessed, no rate limits hit.
- Not a vulnerability scan with a tool. Tools produce noise. This is a human-written audit.
- Not a SEO retainer. SEO comes up where it affects security or structure.
- Not an ongoing service. Single fixed-price engagement. If you want monthly monitoring, that is a separate offering.

---

## Proof

Two recent audits, ready to read:
...
- **ipintegration.com (UK enterprise tech)** — Properly hardened site. The audit found two High-severity items (a WCAG-blocking viewport setting, a missing Content-Security-Policy) and a tight list of minor improvements. Real engineering value, fewer than 16 hours of remediation work.

Both audits used the same checklist. Same severity grading. Same deliverables.
